...
<head>
    ...
    <link rel="stylesheet" href="{{asset('public/css/'.setting("theme_color","primary").'.min.css')}}">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/dropzone/5.8.1/dropzone.min.css" integrity="sha512-jU/7UFiaW5UBGODEopEqnbIAHOI8fO6T99m7Tsmqs2gkdujByJfkCbbfPSN4Wlqlb9TGnsuC0YgUgWkRBK7B9A==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    @yield('css_custom')
</head>

<body class="@if(in_array(setting('language','en'), ['ar','ku','fa','ur','he','ha','ks'])) rtl @else ltr @endif layout-fixed {{setting('fixed_header',false) ? "layout-navbar-fixed" : ""}} {{setting('fixed_footer',false) ? "layout-footer-fixed" : ""}} sidebar-mini {{setting('theme_color')}} {{setting('theme_contrast','')}}-mode" data-scrollbar-auto-hide="l" data-scrollbar-theme="os-theme-dark">
<div class="wrapper">
...
<script src="{{asset('public/vendor/bootstrap-v4-rtl/js/bootstrap.bundle.min.js')}}"></script>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/dropzone/5.8.1/dropzone.min.js" integrity="sha512-QpZR7HNNxb5QQ5+qVXikXT0nDHUEwr0sNLG2pLaFCzpJ7ZqSsNEU8YfzWr9VzX29r7XPTPgASyfTFZeoSJe3sA==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
...
<script src="{{asset('public/js/scripts.min.js')}}"></script>
@stack('scripts')
...
$(".checkbox").change(function() {
    ...
    var coins = $(this).attr('data-coin');
    var user_id = $(this).attr('data-userid');
    if(this.checked) {
        //   alert($(this).attr('data-userid'))
         $.ajax({
               type:'POST',
               url:'{{url("packages_order_status")}}',
               data:{
                "_token":"{{ csrf_token() }}",
                "id": $(this).val(),
                "status": 1,
