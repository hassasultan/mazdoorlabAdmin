...
<div class="d-flex flex-column col-sm-12 col-md-6">
    ...
    @prepend('scripts')
        <script type="text/javascript">
            ...
            @if(isset($hash) && $hash->hasMedia('image'))
                var16110650672130312723ble = {
                    name: "{!! $hash->getFirstMedia('image')->name !!}",
                    size: "{!! $hash->getFirstMedia('image')->size !!}",
                    type: "{!! $hash->getFirstMedia('image')->mime_type !!}",
                    collection_name: "{!! $hash->getFirstMedia('image')->collection_name !!}"
                };
            @endif
            var dz_var16110650672130312723ble = $(".dropzone.image").dropzone({
                    ...
                    init: function () {
                        @if(isset($hash) && $hash->hasMedia('image'))
                        dzInit(this, var16110650672130312723ble, '{!! url($hash->getFirstMediaUrl('image','thumb')) !!}')
                        @endif
                    },
                    ...
                    removedfile: function (file) {
                        dzRemoveFile(
                            file, var16110650672130312723ble, '{!! url("bannerPackageHash/remove-media") !!}',
                            'image', '{!! isset($hash) ? $hash->id : 0 !!}', '{!! url("uplaods/clear") !!}', '{!! csrf_token() !!}'
                        );
                    }
                });
...
@endprepend
    ...
    <div class="form-group align-items-baseline d-flex flex-column flex-md-row">
        ...
        <div class="col-md-9">
            {!! Form::select('status', ['draft'=>'Draft','publish'=>'Publish'], isset($hash)?$hash->status:null, ['data-empty'=>trans("lang.product_category_status"), 'class' => 'select2 required form-control']) !!}
        </div>
    </div>
</div>
@if($customFields)
...
<div class="form-group col-12 d-flex flex-column flex-md-row justify-content-md-end justify-content-sm-center border-top pt-4">
    <button type="submit" class="btn bg-{{setting('theme_color')}} mx-md-3 my-lg-0 my-xl-0 my-md-0 my-2">
        <i class="fa fa-save"></i> {{trans('lang.save')}} {{trans('lang.discover_hashtag')}}
    </button>
    <a href="{!! route('hashtag.index') !!}" class="btn btn-default"><i class="fa fa-undo"></i> {{trans('lang.cancel')}}</a>
</div>
