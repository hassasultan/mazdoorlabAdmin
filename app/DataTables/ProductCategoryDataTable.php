...
class ProductCategoryDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ->editColumn('image', function ($product_category) {
                return getMediaColumn($product_category, 'image');           
            })
            ->editColumn('description', function ($product_category) {
                return getStripedHtmlColumn($product_category, 'description');
            })
            ->editColumn('name', function ($product_category) {
                return $product_category->name;
            })
            ->editColumn('status', function ($product_category) {
                return $product_category->status;
            })
            ->editColumn('updated_at', function ($product_category) {
                return getDateColumn($product_category, 'updated_at');
            })
    ...
    protected function getColumns()
    {
        ...
        $hasCustomField = in_array(ProductCategory::class, setting('custom_field_models', []));
        if ($hasCustomField) {
            $customFieldsCollection = DB::table('custom_fields')->where('custom_field_model', ProductCategory::class)->where('in_table', '=', true)->get();
